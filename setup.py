...
def save_config(config: dict[str, Any]):
    ...
    # Mode section
    lines.append("[mode]")
    lines.append(f"prompt_config_mode = {'true' if config['mode']['prompt_config_mode'] else 'false'}")
    lines.append("")
    ...
    lines.append(f'type_ids_test = "{config["paths"]["data"]["type_ids_test"]}"')
    lines.append("")
...
def setup_logging():
    ...
    current_prompt = config.get("mode", {}).get("prompt_config_mode", True)
    ...
    prompt_mode = Confirm.ask(
        "[key]Show interactive prompts on startup?[/]",
    ...
    config["mode"]["prompt_config_mode"] = prompt_mode
    save_config(config)
...
def view_config():
    ...
    log_table.add_row(
        "Prompt Mode",
        "[success]Yes[/]" if config.get("mode", {}).get("prompt_config_mode", True) else "[hint]No[/]"
    )
